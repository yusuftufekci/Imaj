using Imaj.Core.Entities;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace Imaj.Data.Configurations
{
    public class InvoLineConfiguration : IEntityTypeConfiguration<InvoLine>
    {
        public void Configure(EntityTypeBuilder<InvoLine> builder)
        {
            builder.ToTable("InvoLine");
            builder.HasKey(e => e.Id);
            builder.Property(e => e.Id).HasColumnName("ID").HasColumnType("decimal(12, 0)");
            
            builder.Ignore(e => e.CreatedDate);
            builder.Ignore(e => e.IsActive);

            builder.Property(e => e.InvoiceID).HasColumnType("decimal(10, 0)").IsRequired();
            builder.Property(e => e.FreeFormat).IsRequired();
            builder.Property(e => e.Quantity).HasColumnType("smallint").IsRequired();
            builder.Property(e => e.Price).HasColumnType("decimal(16, 2)").IsRequired();
            builder.Property(e => e.Amount).HasColumnType("decimal(16, 2)").IsRequired();
            builder.Property(e => e.Notes).HasColumnType("ntext").IsRequired();
            builder.Property(e => e.Sequence).HasColumnType("smallint").IsRequired();
            builder.Property(e => e.Deleted).HasColumnType("decimal(12, 0)").IsRequired();
            builder.Property(e => e.SelectFlag).IsRequired();
            builder.Property(e => e.Stamp).HasColumnType("smallint").IsRequired();
            
            builder.Property(e => e.TaxTypeID).HasColumnType("decimal(6, 0)");

            builder.HasOne(d => d.Invoice)
                .WithMany() // Invoice collection tanımı yapılmadı
                .HasForeignKey(d => d.InvoiceID)
                .OnDelete(DeleteBehavior.NoAction);

            builder.HasOne(d => d.TaxType)
                .WithMany()
                .HasForeignKey(d => d.TaxTypeID)
                .OnDelete(DeleteBehavior.NoAction);
        }
    }
}
