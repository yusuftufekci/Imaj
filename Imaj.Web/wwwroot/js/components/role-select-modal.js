function roleSelectModal() {
    return {
        ...
        async search(page) {
            ...
            try {
                const result = await API.get('/User/SearchRoles', params);
                this.items = result.items || [];
                this.totalCount = result.totalCount || 0;
                this.page = result.page || nextPage;
                this.hasMore = this.page * this.filter.pageSize < this.totalCount;
            } catch (error) {
                console.error('Role modal search error:', error);
                this.items = [];
                this.totalCount = 0;
                this.hasMore = false;
                if (window.Toast && typeof window.Toast.error === 'function') {
                    window.Toast.error('Rol listesi alinirken hata olustu.');
                }
            }
        },
        ...
        toggleSelection(role) {
            ...
            const index = this.selectedItems.findIndex(x => Number(x.id) === roleId);
            if (index >= 0) {
        ...
        isSelected(role) {
            const roleId = Number(role.id);
            return this.selectedItems.some(x => Number(x.id) === roleId);
        },

        select(role) {
            this.$dispatch('role-selected', {
                role: role,
                roles: [role],
...
window.roleSelectModal = roleSelectModal;
